...
export async function GET() {
  try {
    const listings = await db.select().from(marketplace_listings).orderBy(desc(marketplace_listings.created_at));
    return NextResponse.json({ listings });
  } catch (err) {
    console.error("GET /api/marketplace error:", err);
    return NextResponse.json({ error: "Failed to fetch listings", details: err?.message || String(err) }, { status: 500 });
  }
}
